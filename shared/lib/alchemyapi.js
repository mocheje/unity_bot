...
var fs = require('fs');
var config = require('../config/alchemy.json')
...
function AlchemyAPI() {
  ...
  //Load the key
	key = config.ALCHEMY_API_KEY;

	//Make sure the key formating looks good
	if (key.length != 40) {
		logger.error('The API key does not appear to be valid. If you do not have a key, register for one at: http://www.alchemyapi.com/api/register.html');
	}
